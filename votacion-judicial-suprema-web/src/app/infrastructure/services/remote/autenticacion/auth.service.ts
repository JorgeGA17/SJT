...
import { LocalStatesService } from '../../local/local-states.service';
...
export class AuthService {
  ...
  constructor(private httpClient: HttpClient, private localStateService: LocalStatesService) { }
  ...
  setRefreshToken(){
    ...
    this.refreshToken(token).subscribe({
      next:(dataRefresh: RefreshTokenResponse)=>{
        if(dataRefresh.codigo===constantes.RESPONSE_COD_EXITO){
          this.setToken(dataRefresh.data.token);
          this.setDatetimeNewToken(newDAte);
        }
        else{
  ...
  handleError(err: HttpErrorResponse): Observable<never> {
    ...
    if (err.error instanceof ErrorEvent) {
      ...
      errorMessage = `Error código:${err.status} \n Mensaje: ${err.message}`;
    }
  ...
  getDiffTimeSession():number{
    try{
      ...
      return Math.floor((sessionNow.getTime() - sessionInit.getTime())/1000);
    }
    catch(e){
        return -1
    }
